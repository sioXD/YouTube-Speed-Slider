...
minSpeed.addEventListener("change", async (e) => {
...
maxSpeed.addEventListener("change", async (e) => {
	let value = parseFloat(e.target.value);
	if (value > parseFloat(minSpeed.value)) {
		chrome.storage.local.set({ "max-speed": value });
	} else {
...
wheelStep.addEventListener("change", async (e) => {
	let value = parseFloat(e.target.value);
	if (value > 0) {
		chrome.storage.local.set({ "wheel-step": value });
	} else {
...
chrome.storage.onChanged.addListener((changes, area) => {
	if (area === "local") {
		const keys = ["min-speed", "max-speed", "wheel-step", "save-speed"];
